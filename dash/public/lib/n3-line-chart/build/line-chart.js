  ...
  'n3utils', '$window', '$timeout', function(n3utils, $window, $timeout) {
    ...
    link = function(scope, element, attrs, ctrl) {
      ...
      scope.update = function() {
        ...
        if(scope.highlightedDays() || angular.isArray(scope.highlightedDays())) {
          var daysToHighlight = scope.highlightedDays();

          $(element).find('.x.axis .tick').each(function(){
            var $text = $(this).find('text');

            var date = $text.text();
            var weekday = moment(date, 'DD.MM.').format('dddd');

            if(daysToHighlight.indexOf(weekday) >= 0){
              $text.attr('fill', '#E26A6A');
            }
          });
        }
  ...
  '$window', '$log', '$rootScope', function($window, $log, $rootScope) {
    return {
      ...
      updateYTooltip: function(svg, _arg, yAxisOptions) {
        ...
        textY = _f ? _f(datum.y, series) : datum.y;
