    ...
    angular.module('nvd3ChartDirectives', [])
        ...
        .directive('nvd3MultiBarChart', [function(){
            return {
                ...
                link: function(scope, element, attrs){
                    function highlightDays(data) {
                      var dates = _.map((data[0] || {}).values || [], function(value) { return value[0] });

                      var $labels = $(element).find('.nv-x .tick');
                      var daysToHighlight = scope.highlightedDays();

                      if(daysToHighlight) {
                        var highlightDaysTimeout = setTimeout(function() {
                          dates.forEach(function(date, index) {
                            var localLocale = moment(date);
                            localLocale.locale('en');
                            var weekday = localLocale.utc().format('dddd');

                            if(daysToHighlight.indexOf(weekday) >= 0 && $labels[index]) {
                              $labels[index].setAttribute('fill', '#E26A6A');
                              $labels[index].style['font-weight'] = 'bold';
                            }
                          });

                          clearTimeout(highlightDaysTimeout);
                        }, 500);
                      }
                    }
                    ...
                    scope.$watch('data', function(data){
                        if(data){
                            ...
                            if(scope.chart){
                                ...
                                var called = scope.d3Call(data, scope.chart);
